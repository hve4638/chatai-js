...
const signatureWEBP = [0x52, 0x49, 0x46, 0x46, null, null, null, null, 0x57, 0x45, 0x42, 0x50] as const;
const signaturePNG = [0x89, 0x50, 0x4E, 0x47, 0x0D, 0x0A, 0x1A, 0x0A];
const signatureJPEG = [0xFF, 0xD8, 0xFF];
export function guessImageExtFromBase64(base64:string):'jpeg'|'png'|'webp'|undefined {
    ...
    if (isHeaderMatch(signatureJPEG)) {
        return 'jpeg';
    }
    else if (isHeaderMatch(signaturePNG)) {
        return 'png';
    }
    else if (isHeaderMatch(signatureWEBP)) {
        return 'webp';
    }
    else {
        return undefined;
